 ...
 */
export function registerIndexerSignal(indexerUrl, signal) {
 ...
 */
export async function gqlRequest(document, variables, indexerUrl, headers) {
 ...
 */
export async function gqlRequestDynamic(query, variables, indexerUrl, headers) {
...
async function sendGraphql(query, variables, indexerUrl, headers) {
    ...
    return GraphqlBoundary.postGraphql(indexerUrl, query, variables, {
        headers,
...
export async function aggregateCount(table, whereType, where, indexerUrl, headers) {
    try {
        ...
        if (!(/aggregate/i.test(msg) && /not found/i.test(msg)))
            throw e;
        const rows = await gqlRequestDynamic(`query CountFallback($where: ${whereType}!) { ${table}(where: $where, limit: ${COUNT_FALLBACK_CAP}) { id } }`, { where }, indexerUrl, headers);
 ...
 */
export function narrowIndexerInvariant(rows) {
    return rows;
}
