...
const PRICE_TIMEFRAMES = { "1m": "M1", "1h": "H1", "1d": "D1" };
const VALID_RESOLUTIONS = new Set(["M1", "H1", "D1"]);
...
export class SomniaMarkets {
    ...
    registry = new Symbols.SymbolRegistry();
     ...
     */
    bookParams = new Map();
    watches = new AsyncCache();
    priceWatches = new AsyncCache();
    channels = new Map();
    unsubscribe = null;
    unsubscribePrices = null;
    constructor(config) {
        const { privateKey, account, walletClient, ...clientConfig } = config;
        this.client = CreateClient.createClient(clientConfig);
    ...
    async loadMarkets(reload = false) {
        ...
        for (const m of rows) {
            if (m.marketType === "BINARY")
                need.add(Markets.lower0x(m.collateral));
            else {
                if (!m.baseSymbol && !m.baseIsNative)
                    need.add(Markets.lower0x(m.baseToken));
                if (!m.quoteSymbol)
                    need.add(Markets.lower0x(m.quoteToken));
            }
        }
        ...
        const codeOf = (token) => this.currencies.get(Markets.lower0x(token))?.code ?? token.slice(2, 8).toUpperCase();
        ...
        for (const m of rows)
            if (m.marketType === "BINARY")
                pools.add(Markets.lower0x(m.poolAddress));
        if (reload)
        ...
        for (const m of rows) {
            ...
            if (m.marketType === "BINARY") {
                ...
                if (!m.baseIsNative)
                    this.currencies.set(Markets.lower0x(m.baseToken), { code: m.baseSymbol ?? codeOf(m.baseToken), decimals: m.baseDecimals });
    ...
    requireBookParams(t, operation) {
        ...
        if (!bp) {
            throw new InvalidInputError(`${operation}: no book parameters for binary market ${t.symbol} (pool ${t.pool}). ` +
    ...
    bookView(t, native) {
        ...
        const lvl = (l) => [Structs.toHumanNum(l.price, d.price), Structs.toHumanNum(l.quantity, d.amount)];
        if (t.market.marketType === "BINARY") {
            const b = native;
    ...
    async fetchOrderBook(ref, limit = 10) {
        const t = this.market(ref);
        const native = t.market.marketType === "BINARY"
    ...
    async fetchTrades(ref, since, limit = 50) {
        ...
        return rows
            ...
            let side = takerBuysBase === undefined ? undefined : takerBuysBase ? "buy" : "sell";
    ...
    async fetchOHLCV(ref, timeframe = "5m", since, limit = 500) {
        ...
        if (!interval)
            throw new InvalidInputError(`unknown timeframe ${timeframe} (have: ${Object.keys(Structs.TIMEFRAMES).join(" ")})`);
        const d = this.decimalsOf(t);
    ...
    async fetchTicker(ref) {
        ...
        const windowed = stats.openPrice24h !== null && stats.high24h !== null && stats.low24h !== null
            ? this.ohlcView(t, 
            // `close` is a placeholder for the shared view; `last` below is
            // derived from the freshest fill instead.
            { open: stats.openPrice24h, high: stats.high24h, low: stats.low24h, close: stats.openPrice24h }, d.price)
            : undefined;
    ...
    async fetchOrders(ref, since, limit = 100, params = {}) {
        ...
        for (const o of rows) {
            ...
            if (!rt)
                continue;
            if (t?.outcome && rt.outcome !== t.outcome)
                continue;
    ...
    mapPortfolioOrder(o, scope) {
        ...
        if (!t)
            return [];
        if (scope?.outcome && t.outcome !== scope.outcome)
            return [];
        const d = this.decimalsOf(t);
        const tsMs = Number(o.placedAtTimestamp) * 1000;
        return [
    ...
    async fetchPortfolioAnalytics(timeframe, params = {}) {
        ...
        const bucketSec = timeframe === "24h" ? 3600 : timeframe === "7d" ? 14_400 : 86_400;
        const firstEventMs = events.length > 0 ? Math.min(...events.map((e) => e.timestamp)) : asOf;
        const windowStartMs = timeframe === "all"
            ? firstEventMs
        ...
        await Promise.all([...touched.values()].map(async (t) => {
            ...
            series.set(t.symbol, rows.map((c) => [Number(c.bucketStart) * 1000, this.priceView(t, c.closePrice, d.price)]));
    ...
    fillToUnifiedTrade(row, scope) {
        ...
        if (scope && rt.pool.toLowerCase() !== scope.pool.toLowerCase())
            return null;
        if (scope?.outcome && rt.outcome !== scope.outcome)
            return null;
        const d = this.decimalsOf(rt);
    ...
    nextTick(key, getNative, map) {
        ...
        const ch = existing ?? { last: UNSET, waiters: [], getNative, map: map };
        if (!existing)
        ...
        return new Promise((resolve) => ch.waiters.push({ resolve: resolve }));
    }
    ensureListener() {
    ...
    async watchOrderBook(ref, limit = 10) {
        ...
        return this.nextTick(`ob:${t.symbol}:${limit}`, () => t.market.marketType === "BINARY"
            ? this.client.getLiveBinaryOrderBook(t.pool, { depth: limit })
            : this.client.getLiveSpotOrderBook(t.pool, { depth: limit }), (native) => ({ symbol: t.symbol, ...this.bookView(t, native), timestamp: Date.now(), info: native }));
    }
    /**
    ...
    async watchTrades(ref, limit = 50) {
        ...
        return this.nextTick(`tr:${t.symbol}:${limit}`, () => this.client.getLiveFills(t.pool, { limit }), (fills) => fills.map((f) => {
            ...
            const side = t.market.marketType === "BINARY"
    ...
    async watchOrders(ref, limit = 100) {
        ...
        await this.ensureWatch(t);
        const d = this.decimalsOf(t);
        return this.nextTick(`ord:${t.symbol}:${addr}`, () => this.client.getLiveUserOrders(t.pool, addr, { limit }), (orders) => orders
            .filter((o) => {
    ...
    async watchPrice(asset) {
        ...
        return this.nextTick(`px:${key}`, () => this.client.getLivePrice(key), (live) => this.toUnifiedPrice(key, live) ?? { symbol: key, price: 0, ema: 0, timestamp: Date.now(), datetime: Structs.toDatetime(Date.now()), info: null });
    }
    /**
    ...
    async createOrder(ref, type, side, amount, price, params = {}) {
        ...
        if (quantity <= 0n) {
            ...
            throw new InvalidInputError(`amount ${amount} is below one lot on ${t.symbol} (lot ${Structs.toHumanNum(lotRaw, d.amount)}) — ` +
    ...
    async crossingPrice(t, side, slippage) {
        const book = this.client.getWatchStatus(t.pool) === "live"
    ...
    async createStopOrder(ref, type, side, amount, triggerPrice, price, params = {}) {
        ...
        const limitPrice = price !== undefined
        ...
        if (result.stopOrderId === undefined) {
            ...
            throw new RpcError("createStopOrder", `stop order tx ${result.hash} landed but PendingOrderCreated was not in the receipt (ABI/deployment drift?) — registry id unknown; recover via the registry directly`);
        }
        const now = Date.now();
        return {
    ...
    async fetchFundingRateHistory(ref, since, limit) {
        ...
        const rows = await this.client.listFundingRateHistory(t.pool, {
            ...
            ...(since != null ? { from: Math.floor(since / 1000), order: "asc" } : {}),
        });
    ...
    async redeem(ref, amount) {
        ...
        const res = await this.trader.redeem({
            ...
            outcomeIdx: bm.winningOutcome == null ? undefined : bm.winningOutcome,
