...
function walkConsumingBase(levels, baseBudget) {
    ...
    for (const [price, qty] of levels) {
        if (price <= 0)
            continue; // not a real crossing; would distort the average
...
function walkConsumingQuote(levels, quoteBudget) {
    ...
    for (const [price, qty] of levels) {
        if (price <= 0)
            continue; // not a real crossing; 0/0 below would NaN-poison the walk
 ...
 *  detect it.
 */
export function estimateMarketOrder(book, side, amount, denomination = "base") {
    if (amount <= 0)
        return null;
    const levels = side === "buy" ? book.asks : book.bids;
    if (levels.length === 0)
        return null;
    const filled = denomination === "quote"
...
export function bookMidPrice(book) {
 ...
 *  than fill deep.
 */
export function fillsWithinSlippage(book, side, amount, slippage, denomination = "base") {
