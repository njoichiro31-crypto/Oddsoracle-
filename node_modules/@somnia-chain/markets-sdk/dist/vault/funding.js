 ...
 */
export const NATIVE_TOKEN_SENTINEL = "0x28f34DeFd2b4CB48d9eE6d89f2Be4Bc601694c00";
/**
 ...
 *  ```
 */
export async function depositVault(w, p) {
    if (p.amount <= 0n)
 ...
 *  ```
 */
export async function depositVaultNative(w, p) {
    if (p.amount <= 0n)
        throw new InvalidInputError("amount must be > 0");
    const gas = p.gas ?? w.defaultGas;
    ...
    if (collateral !== undefined && collateral.toLowerCase() !== NATIVE_TOKEN_SENTINEL.toLowerCase()) {
        throw new InvalidInputError(`pool ${p.vault} does not accept native deposits — its vault token is ${collateral}. ` +
    ...
    return w.execute({
        ...
        abi: TradeAbi.erc20VaultWriteAbi,
        ...(p.owner
            ? { functionName: "depositNativeFor", args: [p.owner] }
            : { functionName: "depositNative", args: [] }),
        value: p.amount,
        gas,
    });
}
