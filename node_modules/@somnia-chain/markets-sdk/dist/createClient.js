 ...
 */
const EMPTY_BINARY_BOOK = { yesBids: [], yesAsks: [], noBids: [], noAsks: [] };
/**
...
export function createClient(config) {
    ...
    if (config.signal)
        IndexerRead.registerIndexerSignal(url, config.signal);
    ...
    const bookParamsFor = (pool) => {
        ...
        return _bookParamsCache.getOrCreate(key, () => Orders.getBinaryBookParams(key, getClient()));
    };
    ...
    const client = {
        ...
        get lend() {
            lendClient ??= Lend.createLendWithDeps({ getConfig, getClient }, config.addresses?.lend ?? {});
            return lendClient;
        },
        ...
        getBinaryPositionPnL: async (account, marketId) => {
            ...
            const readBookTop = async () => {
                try {
                    const book = await Orders.getBinaryOrderBook(market.poolAddress, { decimals: market.quoteDecimals ?? Store.DECIMALS, depth: 1 }, getClient());
            ...
            return DerivedReads.computePositionPnL(events, { balanceYes: BigInt(balances.yes), balanceNo: BigInt(balances.no) }, market, oneCollateral, { bookTop });
        },
        ...
        getPerpStopOrderSomiPayment: (registry) => PerpStops.getPerpStopOrderSomiPayment(registry, getClient()),
        ...
        listFundingRateCandles: (pool, intervalSeconds, opts) => PerpHistory.listFundingRateCandles(pool, intervalSeconds, opts ?? {}, url),
        ...
        isGloballyApproved: (p) => SpotOperatorGrants.isGloballyApproved(p, getClient(), config.addresses?.operatorPermissionsRegistry),
        isApprovedForPool: (p) => SpotOperatorGrants.isApprovedForPool(p, getClient(), config.addresses?.operatorPermissionsRegistry),
        ...
        getMarketOnchain: (marketId) => {
            ...
            return Markets.getMarketOnchain(marketId, { module, settlement: config.addresses?.binarySettlement }, getClient());
        },
        ...
        getErc20Balance: (token, account) => Balances.getErc20Balance(token, account, getClient()),
        getErc20Metadata: (token) => Balances.getErc20Metadata(token, getClient()),
        getErc20Allowance: (token, owner, spender) => Balances.getErc20Allowance(token, owner, spender, getClient()),
        ...
        getSchedulingCost: (def) => OracleHub.getSchedulingCost(def, requireOracleHub(), getClient()),
        earmarkedOf: (operatorId) => OracleHub.earmarkedOf(operatorId, requireOracleHub(), getClient()),
        creditOf: (operatorId) => OracleHub.creditOf(operatorId, requireOracleHub(), getClient()),
        outstandingOf: (operatorId) => OracleHub.outstandingOf(operatorId, requireOracleHub(), getClient()),
        withdrawableOf: (operatorId) => OracleHub.withdrawableOf(operatorId, requireOracleHub(), getClient()),
        payerCreditOf: (payer) => OracleHub.payerCreditOf(payer, requireOracleHub(), getClient()),
        payerOf: (marketId) => OracleHub.payerOf(marketId, requireOracleHub(), getClient()),
        resolveReserve: () => OracleHub.resolveReserve(requireOracleHub(), getClient()),
        quoteCreateMarketValue: (def) => OracleHub.quoteCreateMarketValue(def, requireOracleHub(), getClient()),
