...
class LiveTail {
    ...
    get discoverySources() {
        ...
        const sources = [];
        if (addresses?.marketCreator)
            sources.push(addresses.marketCreator.toLowerCase());
        if (addresses?.binaryModule)
    ...
    async watchAllMarkets(discover) {
        ...
        try {
            if (this.allPools.size === 0)
                ...
                this.ensureSubscriptions(); // maybe the factory just became active
        }
        catch (e) {
            this.allRefs--;
            if (discover)
    ...
    onLogs(logs) {
        ...
        const events = applicable
            .map((l) => this.decode(l))
            .filter((e) => e !== null && !this.processed.has(evKey(e.blockNumber, e.logIndex)))
    ...
    decode(log) {
        ...
        if (!this.watchSet.has(address))
            return null; // not one of our sources
        ...
        if (!LogTopics.isKnownTopic0(LIVE_TOPIC0, log.topics))
            return null;
        let decoded;
        try {
