...
function isMissingView(err) {
    if (!(err instanceof BaseError))
        return false;
    return Boolean(err.walk((e) => e instanceof ContractFunctionZeroDataError || e instanceof ContractFunctionRevertedError));
}
/**
 ...
 */
export const PERP_POOL_FACTORY_MARKET_STATUS_INTERFACE_ID = "0xa874fb70";
/**
 ...
 */
export async function listPerpPoolStatuses(p, client) {
    ...
    const f = { address: p.factory, abi: ReadsAbi.perpPoolFactoryReadAbi, blockNumber };
    ...
    return Promise.all(base.map(async (b) => {
 ...
 */
export async function listTradeablePerpPools(p, client) {
    const statuses = await listPerpPoolStatuses(p, client);
    const undetermined = statuses.filter((s) => s.tradeable === null);
    if (undetermined.length > 0) {
        throw new Error(`cannot determine tradeability for ${undetermined.length} of ${statuses.length} perp markets: ` +
 ...
 */
export async function isPerpPoolRegistered(p, client) {
