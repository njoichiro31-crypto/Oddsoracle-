...
import { buildFundingRateSeries } from "./funding.js";
 ...
 */
export function SomniaMarketsProvider({ client, children, }) {
...
export function useSomniaMarketsClient() {
    const client = useContext(ClientContext);
    if (!client) {
        throw new InvalidInputError("no client in context — wrap your tree in <SomniaMarketsProvider client={exchange.client}>");
    }
    return client;
}
/**
 ...
 */
export function useWatchMarket(pool) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getWatchStatus(pool) : "unwatched"), () => "unwatched");
}
/**
 ...
 */
export function useWatchUser(user) {
    const client = useSomniaMarketsClient();
...
export function useIsTailing() {
    return useLiveStatus().mode === "tailing";
}
const EMPTY_FILLS = [];
const EMPTY_ORDERS = [];
/** Live trade tape for one pool. Watches the pool while mounted. */
export function useLiveFills(pool, limit = 40) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getLiveFills(pool, { limit }) : EMPTY_FILLS), () => EMPTY_FILLS);
}
/**
 ...
 */
export function useLiveUserFills(pool, user, limit = 50) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (user ? client.getLiveUserFills(pool, user, { limit }) : EMPTY_FILLS), () => EMPTY_FILLS);
}
/** One market by pool address (either kind). Watches the pool while mounted. */
export function useLiveMarketByPool(pool) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getLiveMarketByPool(pool) : null), () => null);
}
/**
 ...
 */
export function useLiveMarketByAddress(addr) {
    const client = useSomniaMarketsClient();
    return useSyncExternalStore(client.subscribeLive, () => (addr ? client.getLiveMarketByAddress(addr) : null), () => null);
}
/** `user`'s orders on one pool. Watches the pool while mounted. */
export function useLiveUserOrders(pool, user, limit = 100) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool && user ? client.getLiveUserOrders(pool, user, { limit }) : EMPTY_ORDERS), () => EMPTY_ORDERS);
}
const EMPTY_BOOK = { yesBids: [], yesAsks: [], noBids: [], noAsks: [] };
const EMPTY_SPOT_BOOK = { bids: [], asks: [] };
/**
 ...
 */
export function useLiveBinaryOrderBook(pool, depth = 10) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getLiveBinaryOrderBook(pool, { depth }) : EMPTY_BOOK), () => EMPTY_BOOK);
}
/**
 ...
 *  store).
 */
export function useLiveBinaryOrderBookByMarket(marketId, depth = 10) {
    const client = useSomniaMarketsClient();
    ...
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (id ? client.getLiveBinaryOrderBookByMarket(id, { depth }) : EMPTY_BOOK), () => EMPTY_BOOK);
}
/**
 ...
 *  while mounted.
 */
export function useLiveSpotOrderBook(pool, depth = 12) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getLiveSpotOrderBook(pool, { depth }) : EMPTY_SPOT_BOOK), () => EMPTY_SPOT_BOOK);
}
/**
 ...
 * ```
 */
export function useIndexerQuery(fn, deps) {
    const client = useSomniaMarketsClient();
    ...
    const run = useCallback(() => {
        ...
        fn(client, ctrl.signal).then((data) => {
...
export function usePortfolio(account, opts) {
    return useIndexerQuery((c) => (account ? c.getPortfolio(account, opts) : Promise.resolve(undefined)), [account, opts?.ordersLimit, opts?.tradesLimit, opts?.since]);
}
...
export function useCandles(pool, intervalSeconds, opts) {
    return useIndexerQuery((c) => (pool ? c.getCandles(pool, intervalSeconds, opts) : Promise.resolve([])), [pool, intervalSeconds, opts?.limit, opts?.from, opts?.to]);
}
const EMPTY_FUNDING = [];
/**
 ...
 */
export function useLiveFundingUpdates(pool, limit = 50) {
    const client = useSomniaMarketsClient();
    useWatch(pool ? () => client.watchMarket(pool) : null, pool?.toLowerCase());
    return useSyncExternalStore(client.subscribeLive, () => (pool ? client.getLiveFundingUpdates(pool, { limit }) : EMPTY_FUNDING), () => EMPTY_FUNDING);
}
const EMPTY_SERIES = {
    buckets: [],
    truncated: false,
 ...
 */
export function useFundingRateSeries(pool, intervalSeconds, window) {
    const { from, to, limit = 500 } = window;
    const state = useIndexerQuery(async (c) => {
        if (!pool)
            return EMPTY_SERIES;
    ...
    const seenLiveId = useRef(undefined);
    ...
    useEffect(() => {
        ...
        if (seenLiveId.current === latestLiveId)
            return;
        seenLiveId.current = latestLiveId;
        refetch();
...
export function useMarketFees(marketId) {
    return useIndexerQuery((c) => (marketId ? c.getMarketFees(marketId) : Promise.resolve(undefined)), [marketId]);
}
/** Operator directory (indexer read). Pass `owner`/`enabled` to filter, page with `limit`/`offset`. */
export function useOperators(opts) {
    return useIndexerQuery((c) => c.listOperators(opts), [opts?.owner, opts?.enabled, opts?.limit, opts?.offset]);
}
/**
 ...
 */
export function useMarketCreators(opts) {
    return useIndexerQuery((c) => c.listMarketCreators(opts), [opts?.owner, opts?.operatorId, opts?.venueId, opts?.limit, opts?.offset]);
}
/**
 *  Oracle-adapter directory (indexer read). Pass `owner`/`approved` to filter,
 *  page with `limit`/`offset`.
 */
export function useOracleAdapters(opts) {
    return useIndexerQuery((c) => c.listOracleAdapters(opts), [opts?.owner, opts?.approved, opts?.limit, opts?.offset]);
}
const EMPTY_MARKETS = [];
/**
...
export function useLiveMarkets() {
    const client = useSomniaMarketsClient();
    return useSyncExternalStore(client.subscribeLive, () => client.getLiveMarkets(), () => EMPTY_MARKETS);
}
 ...
 */
export function useWatchPrice(asset) {
    const client = useSomniaMarketsClient();
    const key = asset?.toUpperCase();
    useWatch(key ? () => client.watchPrice(key) : null, key);
    return useSyncExternalStore(client.subscribePrices, () => (key ? client.getPriceStatus(key) : "unwatched"), () => "unwatched");
}
/**
...
export function useLivePrice(asset) {
    ...
    const key = asset?.toUpperCase();
    useWatch(key ? () => client.watchPrice(key) : null, key);
    return useSyncExternalStore(client.subscribePrices, () => (key ? client.getLivePrice(key) : null), () => null);
}
/**
 ...
 */
export function useLivePriceFeedInfo(asset) {
    const client = useSomniaMarketsClient();
    const key = asset?.toUpperCase();
    useWatch(key ? () => client.watchPrice(key) : null, key);
    return useSyncExternalStore(client.subscribePrices, () => (key ? client.getLivePriceFeedInfo(key) : null), () => null);
}
/** The live tick tape of one asset, newest first. Watches the feed while mounted. */
export function useLivePriceTicks(asset, limit = 100) {
    const client = useSomniaMarketsClient();
    const key = asset?.toUpperCase();
    useWatch(key ? () => client.watchPrice(key) : null, key);
    return useSyncExternalStore(client.subscribePrices, () => (key ? client.getLivePriceTicks(key, { limit }) : EMPTY_TICKS), () => EMPTY_TICKS);
}
 ...
 */
export function useLendAccount(account) {
    return useIndexerQuery((c) => (account ? c.lend.getAccount(account) : Promise.resolve(undefined)), [account]);
}
